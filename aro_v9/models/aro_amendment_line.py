...
class AroAmendmentLine(models.Model):
    _name = 'aro.amendment.line'
    ...
    name = fields.Char(string='Amendment number')
    starting_date = fields.Date(string='Starting date',
    default=fields.Date.context_today,required=True)
    ending_date = fields.Date(string='Ending date',
    default=fields.Date.context_today,required=True)
    creating_date = fields.Date(string='Creating date',
    default=fields.Date.context_today,required=True)
    is_last_situation = fields.Boolean(string='Is the last situation', default=False)
    is_validated = fields.Boolean(string='Is validated', default=False)
    capital = fields.Float(string='Capital', digit_compute=dp.get_precision('account'))
    eml = fields.Float(string='Expected maximum loss', digit_compute=dp.get_precision('account'))
    emission_id = fields.Many2one(comodel_name='aro.type.emission', string='Emission type', default=lambda self: self.env.ref('aro_v9.devis').id)
    risk_line_ids = fields.One2many(comodel_name='aro.insurance.subscription.risk.line', inverse_name='amendment_line_id', string='Risks Type')
    parent_id = fields.Many2one(comodel_name='aro.amendment.line', string='Parent', help='Inherited Amendment')
    invoice_id = fields.Many2one(comodel_name='account.invoice', string='Invoice', readonly=True)
    comment = fields.Text(string='Comment', help='Some of your note')
    apporteur_id = fields.Many2one(comodel_name='res.apporteur', string='Broker', help='Initial broker of the contract')
    is_temporary = fields.Boolean(string='Temporary', help='Check it if it is an temporary amendment')

    @api.multi
    def confirm_quotation(self):
        """Confirm Quotation and move state to valid new contract"""
        if not self._context.get('validate', False):
            self.emission_id = self.env.ref('aro_v9.affaire_nouvelle').id
            self.subscription_id.status_id = self.env.ref('aro_v9.status_en_cours').id
    ...
    def copy(self, default=None):
        ...
        for risk_line_id in self.risk_line_ids:
            if not new_risk_line:
                ...
                new_risk_line.write({'amendment_line_id': res.id})
            else:
                new_risk_line_buf = risk_line_id.copy()
                new_risk_line_buf.write({'amendment_line_id': res.id})
    ...
    def _get_all_value(self):
        ...
        om_fields = {
            'warranty_line_ids': ['name', 'warranty_id', 'risk_id', 'types'],
        ...
        if not self._context.get('default', False):
            ...
            for risk_line_id in self.risk_line_ids:
                ...
                for om_warranty in om_warrantys:
                    ...
                    if self._context.get('version_type', False):
                        om_warranty['types'] = 'untouched' if om_warranty['types'] == 'new' else om_warranty['types']
    ...
    def create(self, vals):
        subscription_obj = self.env['aro.insurance.subscription']
        subscription_id = subscription_obj.browse(self._context.get('default_subscription_id'))
        ...
        if self._context.get('version_type') in ['renew', 'amendment', 'reinstatement']:
            subscription_id.write({'status_id':self.env.ref('aro_v9.status_en_cours').id})
        elif self._context.get('version_type') == 'suspend':
            subscription_id.write({'status_id':self.env.ref('aro_v9.status_suspendu').id})
        elif self._context.get('version_type') == 'terminate':
            subscription_id.write({'status_id':self.env.ref('aro_v9.status_resilie').id})
    ...
    def generate_invoice(self):
        ...
        if self.invoice_id:
            ...
            compl_line = {
                'price_unit': self.subscription_id.produit_assurance_id.amount_accessories
            }
            ...
            ctx_vals = {
                ...
                'default_amendment_line_id': self.id,
    ...
    def onchange_temporary(self):
        if self._context.get('version_type') == 'amendment':
            if self.is_temporary:
                self.emission_id = self.env.ref('aro_v9.avenant_temporaire').id
            else:
                self.emission_id = self.env.ref('aro_v9.avenant').id

    @api.onchange('starting_date')
    def onchange_starting_date(self):
        date_comp = self.subscription_id.get_end_date(self.starting_date)
        self.ending_date = date_comp.get('end_date')
